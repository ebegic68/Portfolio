...
function validate_user_registration(){
    ...
    if($_SERVER['REQUEST_METHOD']=="POST"){
        ...
        if(strlen($first_name)<3){
            $errors[]= "Your First Name cannot be less then 3 charasters!";
        }
        if(strlen($last_name)<3){
            $errors[]= "Your Last Name cannot be less then 3 charasters!";
        }
        if(strlen($username)<3){
            $errors[]= "Your Username cannot be less then 3 charasters!";
        }
        if(strlen($username)>20){
            $errors[]= "Your First Name cannot be bigger then 20 charasters!";
        }
        if(email_exists($email)){
            $errors[]= "Sorry that Email is already is taken";
        }
        if(user_exists($username)){
            $errors[]= "Sorry that Userame is already is taken";
        }
        if(strlen($password)<8){
            $errors[]="Your Password cannot be less then 8 charasters!";
        }
...
function create_user($first_name, $last_name, $username, $email, $password){
        ...
        confim(query($sql));
        set_message("You have been successfuly registed! Please log in!");
        redirect("login.php");
...
function user_profile_image_upload(){
    if($_SERVER['REQUEST_METHOD']=="POST"){
        ...
        if($_FILES['profile_image_files']['size']>5000000){
...
function create_post(){
    ...
    if($_SERVER['REQUEST_METHOD']== "POST"){
        ...
        if(!empty($errors)){
            ...
            confim(query($sql));
            set_message("You added the post!");
            redirect("index1.php");
        }
    }
}
    ...
    if($result->num_rows>0){
        while($row=$result->fetch_assoc()){
            ...
            echo "<div class='post'>
                ...
                <div class='likes'>Likes: <b id='likes_"  . $row['id'] .  "'>" . $row['likes'] . "</b> <button data-post_id='"  .  $row['id']  .  "' onclick='like_post(this)'>LIKE</button>
                </div>
